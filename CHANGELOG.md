...
- **Breaking:** `BoolQuery` now checks the values of its two options and throws `InvalidSearchParametersException` naming the key and the given type: `minimum_should_match` must be an int or a string (`1`, `"75%"`, `"3<90%"`) and `boost` an int, a float or a numeric string. A clause object under an option key (`'boost' => Must::make(...)`) used to pass validation and was then sent twice, as the clause and as the option's value, which OpenSearch rejects. It is marked breaking because of one narrow case that did produce a valid request: without a Should clause `minimum_should_match` is dropped, so a non-Should clause or an invalid value (such as a float) under that key used to go through silently and now throws. `null` under either key still means "not set" and is left out.
- **Breaking:** the third parameter of `Regexp::make()` and of its constructor was renamed from `$casInsensitive` to `$caseInsensitive`, the spelling `Prefix` and `Wildcard` already use. Only calls that pass it as a named argument (`casInsensitive: true`) must change to `caseInsensitive: true`; positional calls and the query produced are unchanged.
...
- `DisMax`, `FunctionScore` and the `Filters`, `Composite` and `MultiTerms` aggregations now check the items of the list they take when they are constructed. An item of the wrong kind used to fail with a PHP `Error` ("Call to a member function toOpenSearchQuery() on ...") or `TypeError` only when the request was built; it now throws the package exception naming the item's type and key — `InvalidSearchParametersException` from `DisMax` (a query that is neither a `SearchQueryType` nor a `BoolQuery`) and `FunctionScore` (a function that is not an array, or a `filter` object that is not a query), `InvalidAggregationParametersException` from `Filters` (a filter that is not a query), `Composite` (a source that is not a field name, an array or an aggregation) and `MultiTerms` (a field that is not a string). `DisMax`, `Filters` and `Composite` also throw for an empty list, and `MultiTerms` for fewer than two fields, which OpenSearch rejects; `FunctionScore` still accepts an empty list of functions, which OpenSearch accepts. Input that used to build a valid request is unaffected, with one narrow exception: a `MultiTerms` field given as an int or a `Stringable` object used to be converted to a string and now throws, so pass field names as strings. Otherwise only code catching `\Error` for these cases sees a different exception.
...
- `BucketSort` now throws `InvalidAggregationParametersException` when it is created with a `size` below 1 or a `from` below 0. OpenSearch rejects both ("[size] must be a positive integer", "[from] must be a non-negative integer"), so these requests already failed, only later and with an HTTP error from the cluster.
